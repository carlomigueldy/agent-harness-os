...
3. **Create worktrees off the green base.** One per epic, following the [worktree protocol](../../../.agents/workflows/worktree-sessions.md): `git worktree add .agents/worktrees/feat/<epic> -b feat/<epic> <base>`. Set the repo-local commit identity (shared across worktrees). **Provision gitignored test deps** into each worktree (copy, do not commit).
